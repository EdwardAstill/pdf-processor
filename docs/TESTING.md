...
- **Formula candidates** — tests in `src/formula/` cover centered equation-number detection, prose rejection, and bbox clamping; `tests/formulas.rs` runs the real CLI against `math-number-theory.pdf` and checks `--debug-formulas` JSON, rendered formula crops, and local warning behavior.
